...
process.stdin.on('end', () => {
  ...
  try {
    ...
    // Support both Claude Code (tool_name) and Copilot (toolName) input formats
    const toolName = data.tool_name || data.toolName;

    // Only scan Write/Edit (Claude Code) or edit (Copilot's mapped tool name)
    if (toolName !== 'Write' && toolName !== 'Edit' && toolName !== 'edit') {
      process.exit(0);
    }

    // Support both Claude Code (tool_input object) and Copilot (toolArgs JSON string)
    let toolInput = data.tool_input;
    if (!toolInput && data.toolArgs) {
      try { toolInput = JSON.parse(data.toolArgs); } catch (e) { process.exit(0); }
    }

    // Copilot's edit tool uses newPath as the file path key
    const filePath = toolInput?.file_path || toolInput?.path || toolInput?.newPath || '';
    ...
    const content = toolInput?.content || toolInput?.new_string || toolInput?.newContent || '';
    if (!content) {
      process.exit(0);
    }
