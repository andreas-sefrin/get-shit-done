...
describe('Codex install hook configuration (e2e)', () => {
  ...
  test('Codex install does not write the removed update-check hook into config.toml', () => {
    runCodexInstall(codexHome);

    const configContent = readCodexConfig(codexHome);
    assert.ok(!configContent.includes('gsd-check-update.js'), 'config.toml should not reference gsd-check-update.js');
  });
  ...
  test('fresh CODEX_HOME enables codex_hooks without draft root defaults', () => {
    ...
    assert.ok(!content.includes('# GSD Hooks\n[[hooks]]\nevent = "SessionStart"\n'), 'does not write the removed GSD SessionStart hook block');
    assert.strictEqual(countMatches(content, /^codex_hooks = true$/gm), 1, 'writes one codex_hooks key');
    assert.strictEqual(countMatches(content, /gsd-check-update\.js/g), 0, 'writes no GSD update hook');
  ...
  test('existing LF config without [features] gets one features block and preserves user content', () => {
    ...
    assert.strictEqual(countMatches(content, /gsd-check-update\.js/g), 0, 'adds no GSD update hook');
    assertNoDraftRootKeys(content);
  });
  ...
  test('existing empty [features] and codex_hooks = false are normalized and remain idempotent', () => {
    ...
    assert.ok(content.includes('command = "echo custom"'), 'preserves custom hook');
    assert.strictEqual(countMatches(content, /gsd-check-update\.js/g), 0, 'does not emit a removed GSD update hook');
    assertNoDraftRootKeys(content);
  });
  ...
  test('quoted [features] headers are recognized as the existing features table', () => {
    ...
    assert.strictEqual(countMatches(content, /gsd-check-update\.js/g), 0, 'keeps no GSD update hook');
    assertNoDraftRootKeys(content);
  });
  ...
  test('quoted table headers containing # are parsed without treating # as a comment start', () => {
    ...
    assert.strictEqual(countMatches(content, /^codex_hooks = true$/gm), 1, 'adds one codex_hooks key');
    assert.strictEqual(countMatches(content, /gsd-check-update\.js/g), 0, 'remains idempotent with no GSD hook block');
    assertNoDraftRootKeys(content);
  });
  ...
  test('existing dotted features config stays dotted and does not grow a [features] table', () => {
    ...
    assert.strictEqual(countMatches(content, /gsd-check-update\.js/g), 0, 'adds no GSD update hook for dotted codex_hooks');
    assertNoDraftRootKeys(content);
  });
  ...
  test('quoted dotted codex_hooks keys stay dotted and are normalized without duplication', () => {
    ...
    assert.strictEqual(countMatches(content, /gsd-check-update\.js/g), 0, 'adds no GSD update hook for quoted dotted codex_hooks');
    assertNoDraftRootKeys(content);
  });
  ...
  test('mixed-EOL configs use the first newline style for inserted Codex content', () => {
    ...
    assert.ok(!content.includes('# GSD Hooks\n[[hooks]]\nevent = "SessionStart"\n'), 'does not write the removed GSD hook block');
    ...
    assert.strictEqual(countMatches(content, /gsd-check-update\.js/g), 0, 'does not duplicate a removed GSD hook block');
    assertNoDraftRootKeys(content);
  });
});
